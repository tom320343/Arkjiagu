...
static const int GUARD_BLOCK_FLAG_LAYER_1 = 0x100;
static const int GUARD_BLOCK_FLAG_LAYER_2 = 0x200;
static const int GUARD_BLOCK_FLAG_LAYER_3 = 0x400;
static const int GUARD_BLOCK_FLAG_MULTI_LAYER = GUARD_BLOCK_FLAG_LAYER_1 | GUARD_BLOCK_FLAG_LAYER_2 | GUARD_BLOCK_FLAG_LAYER_3;
...
static const unsigned char GUARD_INDEX_MAGIC[4] = {0x71, 0x32, 0xA6, 0x4D};
static const unsigned char GUARD_BLOCK_MAGIC[4] = {0x63, 0x19, 0xB7, 0x52};
static const unsigned char GUARD_FOOTER_MAGIC[4] = {0x5C, 0xE1, 0x38, 0x90};
...
static std::vector<unsigned char> buildPayloadIndexPlainBytes(
        const std::vector<GuardPayloadIndexEntry> &entries
) {
    std::vector<unsigned char> out;

    out.push_back(GUARD_INDEX_MAGIC[0]);
    out.push_back(GUARD_INDEX_MAGIC[1]);
    out.push_back(GUARD_INDEX_MAGIC[2]);
    out.push_back(GUARD_INDEX_MAGIC[3]);
...
static void addPayloadIndexEntry(
        std::vector<GuardPayloadIndexEntry> &indexTable,
        int type,
        int index,
        int offset,
        int size,
...
static bool hideGuardFeaturesEnabled() {
    const char *envVal = getenv("GUARD_HIDE_FEATURES");
...
static void maskArray(unsigned char *arr, int len) {
    if (!hideGuardFeaturesEnabled()) return;
...
static const DisguiseMagic *getDisguiseConfig() {
    const char *envVal = getenv("GUARD_DISGUISE_TYPE");
...
static void shuffleIndexTable(std::vector<GuardPayloadIndexEntry> &items) {
    if (items.size() <= 1) {
        return;
    }
...
static std::vector<unsigned char> buildEncryptedBlockBytes(
    ...
    if (useExternalKey) {
        key = externalKey;
        flags = GUARD_BLOCK_FLAG_SIGN_KEY;
    } else {
        if (!fillRandomBytes(randomKey, sizeof(randomKey))) {
            return result;
        }
        key = randomKey;
        flags = GUARD_BLOCK_FLAG_RANDOM_KEY;
    }

    flags |= GUARD_BLOCK_FLAG_MULTI_LAYER;
    ...
    result.push_back(GUARD_BLOCK_MAGIC[0]);
    result.push_back(GUARD_BLOCK_MAGIC[1]);
    result.push_back(GUARD_BLOCK_MAGIC[2]);
    result.push_back(GUARD_BLOCK_MAGIC[3]);
...
static std::vector<unsigned char> buildPayloadFooterBytes(
    ...
    out.push_back(GUARD_FOOTER_MAGIC[0]);
    out.push_back(GUARD_FOOTER_MAGIC[1]);
    out.push_back(GUARD_FOOTER_MAGIC[2]);
    out.push_back(GUARD_FOOTER_MAGIC[3]);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass mainClazz = env->FindClass("com/apk/guard/GuardMainActivity");
    ...
    jclass vmpUtilsClazz = env->FindClass("com/apk/guard/vm/VmpUtils");
