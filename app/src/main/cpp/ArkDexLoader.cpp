...
static bool check_tracer_pid() {
    ...
    if (tracer_pid != 0) {
        LOGE("[ArkDexLoader] 检测到调试器, TracerPid=%d", tracer_pid);
        return false;
    }
...
/* ================================================================
 * 三层解密函数 — 对应 ArkTool 的三层加密
 *
 * 加密顺序 (ArkTool 端): Layer1(XOR) -> Layer2(分块) -> Layer3(字节混淆)
...
static std::vector<unsigned char> decrypt_three_layers(
    ...
    LOGI("[ArkDexLoader] 开始三层解密, 数据大小=%zu", cipher_len);

    /* Layer 3: 字节混淆逆操作 */
    decrypt_layer3(result.data(), cipher_len);
    LOGI("[ArkDexLoader] Layer3 字节混淆解除完成");
    ...
    LOGI("[ArkDexLoader] Layer2 AES 分块解密完成");

    /* Layer 1: XOR 解密 */
    decrypt_layer1(result.data(), cipher_len, xor_key, xor_key_len);
    LOGI("[ArkDexLoader] Layer1 XOR 解密完成");
...
static jobjectArray loadDexBuffersFromSelfClassesDexBySourceDir(JNIEnv *env, jstring sourceDirJ) {
    ...
    ArkPayloadIndexEntry appEntry, factoryEntry;
    ...
    for (size_t i = 0; i < indexEntries.size(); i++) {
        ...
        if (entry.type != ARK_BLOCK_TYPE_DEX) continue;
...
bool LoaderDEX(JNIEnv *env, jobject context) {
    ...
    if (s_anti_debug_enabled && !check_tracer_pid()) {
        LOGE("[ArkDexLoader] 检测到调试器, 拒绝加载 DEX");
        exit(0);
    }
