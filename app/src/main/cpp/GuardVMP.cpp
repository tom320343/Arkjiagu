...
#define LOG_TAG "GuardVMP_GuardVMP"
...
/* 伪装类型枚举 */
enum GuardDisguiseType {
    GUARD_DISGUISE_DEFAULT    = 0,
    GUARD_DISGUISE_ENTERPRISE = 1,
    GUARD_DISGUISE_FINTECH    = 2,
    GUARD_DISGUISE_GAME       = 3,
    GUARD_DISGUISE_SOCIAL     = 4,
    GUARD_DISGUISE_ECOMMERCE  = 5,
    GUARD_DISGUISE_CUSTOM     = 99
};

/* 伪装备配置的锁 */
static pthread_mutex_t g_disguise_mutex = PTHREAD_MUTEX_INITIALIZER;
static GuardDisguiseType g_disguise_type = GUARD_DISGUISE_DEFAULT;
...
static const char *g_default_section_text =
    "安卓加固 v3.0 - APK离线保护工具 - 支持DEX加密/VMP虚拟化/签名校验。";
 ...
 * ================================================================ */
static const char *get_section_text_for_disguise(GuardDisguiseType type) {
    switch (type) {
        case GUARD_DISGUISE_DEFAULT:
            return g_default_section_text;
        case GUARD_DISGUISE_ENTERPRISE:
            return g_enterprise_section_text;
        case GUARD_DISGUISE_FINTECH:
            return g_fintech_section_text;
        case GUARD_DISGUISE_GAME:
            return g_game_section_text;
        case GUARD_DISGUISE_SOCIAL:
            return g_social_section_text;
        case GUARD_DISGUISE_ECOMMERCE:
            return g_ecommerce_section_text;
        case GUARD_DISGUISE_CUSTOM:
...
/* ================================================================
 * 自定义 section 变量
 * 使用延迟初始化机制，在 GuardVMP_OnLoad 时根据伪装类型设置内容
 * ================================================================ */

/* 使用固定的 section 名称，内容在运行时填充 */
#define GUARD_CUSTOM_SECTION_NAME ".安卓加固 v3.0 - APK离线保护工具 - 支持DEX加密/VMP虚拟化/签名校验。"
...
/*
 * 初始化 section 内容
 * 必须在 GuardVMP_OnLoad 中调用，早于任何对 g_guard_vmp_section 的引用
 */
static void init_section_content() {
    ...
    /* 尾部固定字符串 */
    const char *tail = "APK Guard v3.0";
    ...
    LOGI("[GuardVMP] 伪装类型=%d, section内容已初始化, 长度=%zu",
...
__attribute__((section(GUARD_CUSTOM_SECTION_NAME)))
const char g_guard_vmp_section[] = "";

extern "C"
__attribute__((used))
__attribute__((visibility("default")))
const void *ArkVmpKeepCustomSection() {
    return g_guard_vmp_section;
}
 ...
 *
 * 在 Java 层调用:
 *   GuardVMP.nativeSetDisguiseType(int type)
 *   或
 *   GuardVMP.nativeSetDisguiseType(int type, String customText)
 * ================================================================ */

static JavaVM *g_vm = NULL;

extern "C" void GuardVMP_SetContext(JNIEnv *env, jobject context) {
    VmpRuntime_SetContext(env, context);
}

/*
 * GuardVMP_SetDisguiseType
 ...
 *
 * 注意: 此函数应在 GuardVMP_OnLoad 之前调用以确保 section 内容正确配置。
 *       如果在 OnLoad 之后调用，已编译的 section 内容不会改变，
 *       仅影响运行时上下文。
 */
extern "C"
JNIEXPORT void JNICALL
Java_com_ark_safe_GuardVMP_nativeSetDisguiseType(
    JNIEnv *env, jclass clazz, jint type, jstring customText) {

    pthread_mutex_lock(&g_disguise_mutex);

    GuardDisguiseType new_type;
    switch (type) {
        case 0:  new_type = GUARD_DISGUISE_DEFAULT;    break;
        case 1:  new_type = GUARD_DISGUISE_ENTERPRISE; break;
        case 2:  new_type = GUARD_DISGUISE_FINTECH;    break;
        case 3:  new_type = GUARD_DISGUISE_GAME;       break;
        case 4:  new_type = GUARD_DISGUISE_SOCIAL;     break;
        case 5:  new_type = GUARD_DISGUISE_ECOMMERCE;  break;
        case 99: new_type = GUARD_DISGUISE_CUSTOM;     break;
        default: new_type = GUARD_DISGUISE_DEFAULT;    break;
    }

    LOGI("[GuardVMP] 伪装类型变更: %d -> %d", (int)g_disguise_type, (int)new_type);

    g_disguise_type = new_type;

    /* 处理自定义文本 */
    if (new_type == GUARD_DISGUISE_CUSTOM && customText != NULL) {
        ...
        if (chars != NULL) {
            ...
            LOGI("[GuardVMP] 自定义伪装文本已设置, 长度=%zu",
                 g_custom_section_text.length());
        }
    } else if (new_type == GUARD_DISGUISE_CUSTOM) {
...
/*
 * GuardVMP_SetDisguiseType (简化版本)
 * 仅设置伪装类型，不带自定义文本
 */
extern "C"
JNIEXPORT void JNICALL
Java_com_ark_safe_GuardVMP_nativeSetDisguiseTypeSimple(
    JNIEnv *env, jclass clazz, jint type) {
    Java_com_ark_safe_GuardVMP_nativeSetDisguiseType(env, clazz, type, NULL);
}

/*
 * GuardVMP_GetDisguiseType
 * 获取当前伪装类型
 */
extern "C"
JNIEXPORT jint JNICALL
Java_com_ark_safe_GuardVMP_nativeGetDisguiseType(
...
static JNINativeMethod g_vmp_jni_methods[] = {
    {"nativeSetDisguiseType",       "(ILjava/lang/String;)V", (void *)Java_com_ark_safe_GuardVMP_nativeSetDisguiseType},
    {"nativeSetDisguiseTypeSimple", "(I)V",                   (void *)Java_com_ark_safe_GuardVMP_nativeSetDisguiseTypeSimple},
    {"nativeGetDisguiseType",       "()I",                    (void *)Java_com_ark_safe_GuardVMP_nativeGetDisguiseType},
};
...
extern "C" jint GuardVMP_OnLoad(JavaVM *vm) {
    g_vm = vm;
    ...
    /* 注册伪装类型管理 JNI 方法 */
    jclass vmpClass = env->FindClass("com/apk/guard/safe/GuardVMP");
    if (vmpClass != NULL) {
    ...
    if (stubClass == NULL) {
        env->ExceptionClear();
        LOGE("[GuardVMP] 找不到VM调用入口类：%s", className.c_str());
        return JNI_ERR;
    }
    ...
    if (env->RegisterNatives(stubClass, g_methods, methodCount) != JNI_OK) {
        env->ExceptionClear();
        LOGE("[GuardVMP] RegisterNatives失败，目标类：%s", className.c_str());
        return JNI_ERR;
    }

    LOGI("[GuardVMP] 注册成功，目标类：%s, 伪装类型=%d",
