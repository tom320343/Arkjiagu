...
static bool GuardEnvGuard_DetectXp(JNIEnv *env, jobject context) {
...
static bool GuardEnvGuard_DetectRoot() {
...
static bool GuardEnvGuard_DetectClone() {
    std::string procMaps = DEC(STR_PROC_MAPS);
...
static bool GuardEnvGuard_DetectHookFramework(const std::string &line) {
...
static bool GuardEnvGuard_DetectDebugger() {
...
static bool GuardEnvGuard_DetectAll(JNIEnv *env, jobject context) {
    if (GuardEnvGuard_DetectRoot()) {
        LOGE("检测到Root环境");
        return true;
    }

    if (GuardEnvGuard_DetectClone()) {
        LOGE("检测到分身/双开环境");
        return true;
    }

    if (GuardEnvGuard_DetectDebugger()) {
        LOGE("检测到调试器");
        return true;
    }

    if (GuardEnvGuard_DetectXp(env, context)) {
...
static bool GuardEnvGuard_CheckAndLoad_Impl(JNIEnv *env, jobject context) {
    if (env == nullptr || context == nullptr) {
        return false;
    }

    GuardDexLoaderFunc loaderFunc = GuardDexLoader_GetEntry();
    if (loaderFunc == nullptr) {
        return false;
    }

    if (GuardEnvGuard_DetectAll(env, context)) {
        return false;
    }
 ...
 */
static bool GuardEnvGuard_StartRealApplication_Impl(JNIEnv *env, jobject context) {
    if (env == nullptr || context == nullptr) {
        return false;
    }
    ...
    GuardRealApplicationStarterFunc starterFunc =
            GuardDexLoader_GetStartRealApplicationEntry();
...
GuardEnvGuardFunc GuardEnvGuard_GetEntry() {
    volatile GuardEnvGuardFunc fn = GuardEnvGuard_CheckAndLoad_Impl;
    return fn;
}

GuardEnvGuardFunc GuardEnvGuard_GetStartRealApplicationEntry() {
    volatile GuardEnvGuardFunc fn = GuardEnvGuard_StartRealApplication_Impl;
    return fn;
}